...
const buildSystemPrompt = (existingType?: string): string =>
  [
    ...
    '',
    'Vega here means Vega-Lite only (no raw Vega). Decide in this order:',
    '1. If the request needs a diagram Vega-Lite cannot express — Sankey / flow / alluvial diagrams, network / node-link graphs, chord diagrams, or maps — choose "lens" with the closest standard chart (e.g. a bar chart of the top source→destination combinations) so the user still gets a working visualization.',
    '2. Otherwise, if the user explicitly asks for a Vega or Vega-Lite visualization, choose "vega" — even when a Lens chart type could also express the request.',
    '3. Otherwise prefer Lens. Choose "lens" and the most appropriate chart type whenever one of the available Lens chart types can express the request:',
    getChartTypeSelectionPromptContent(),
    '',
    '   When the user has not asked for Vega, choose "vega" only when no Lens chart type fits but Vega-Lite can, for example:',
